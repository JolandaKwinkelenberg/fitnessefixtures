...
//	protected String return_message = Constants.NOT_INITIALIZED;
	private String workflowResult, FirstErr, LastErr, FirstErrCode, RowsTarget, RowsSource, Duration = Constants.NOT_INITIALIZED;
	private int NumSrcFailedRows, NumSrcSuccessRows, NumDuration, NumTables, NumTgtFailedRows, NumTgtSuccessRows, NumTransErrors, NumEndTime, NumStartTime=0;
    ...
    private void setError(String errCode, String errMsg) {
       ...
       FirstErr =errMsg;
        log("setError",Constants.INFO,"set error code","FirstErrCode set to >" +errCode +"< and FirstErr set to >" + errMsg);
    }
    ...
    public String workflowSuccessful () throws WorkflowStopTest {
        ...
        logMessage="starting workflow =>" +folder +"/" +workflowName +"< in domain =>"+ domainName + " on IS =>" + serviceName + "<.";
        log(myName, Constants.INFO, myArea, logMessage);
        try {
        	...
        	logMessage="Exception starting workflow =>" +folder +"/" +workflowName +"< in domain =>"+ domainName + " on IS =>" + serviceName + "<.";
        	log(myName, Constants.ERROR, myArea, logMessage);
        ...
        try {
    	  diInterface.waitTillWorkflowComplete(wfRequest, sessHeader);
        	...
        	logMessage="Exception waiting on completion of workflow =>" +folder +"/" +workflowName +"< in domain =>"+ domainName + " on IS =>" + serviceName + "<.";
        	log(myName, Constants.ERROR, myArea, logMessage);
        	logMessage="Exception =>" +e.toString() +"<.";
        	log(myName, Constants.FATAL, myArea, logMessage);
                workflowResult=Constants.FATAL;
            if (Constants.YES.equals(abortYesNo)) {
        	throw new WorkflowStopTest("Waiting on workflow completion exception occurred on workflow >=" +folder+"/"+workflowName+"<.");
            }
        }
